package com.icool.reader.component.reader.manager;
...
//                        }
        ...
        int totalSize = pages.size();
        //设置当前页的进度
        for (int index = 0; index < totalSize; index++) {
            //此处关系到书签页索引的计算
            float progress = (index + 1f) / totalSize;
            pages.get(index).setProgress(progress);
            pages.get(index).setTotalPageNum(totalSize);
        }
        //设置书签页
        String storyId = chapter.getStoryid();
        String chapterId = chapter.getChapterid();
        List<BookMarkBean> bookMarkList = IReaderPersistence.queryBookMarkList(storyId, chapterId);
        if (totalSize > 0 && bookMarkList != null && !bookMarkList.isEmpty()) {
            for (BookMarkBean mark : bookMarkList) {
                //这个progress其实就是添加标签的时候页面的progress
                float progress = mark.getProgress();
                int index = (int) (progress * totalSize - 1);
                if (index < 0) index = 0;
                PageData pageData = pages.get(index);
                pageData.setMark(true);
                pageData.setMarkProgress(mark.getProgress());
            }
        }
        return pages;
    }
