...
public class PageRespository {
    ...
    public boolean pre(Bitmap curBitmap, Bitmap nextBitmap) {
        ...
        if (mCurPageList != null && !mCurPageList.isEmpty() && preIndex >= 0) {
     ...
     * 需要回调当前进度
     * <p>
     * int curIndex = (int) (total * progress - 1);
     * 计算 是因为  {@link PageManager#generatePages} 方法在计算进度时在索引的基础上+1计算的
     *
     * @param progress 当前阅读进度
     */
    public void directPageByProgress(float progress) {
        if (mCurPageList == null || mCurPageList.isEmpty()) return;
        int total = mCurPageList.size();
        int curIndex = (int) (total * progress - 1);
        if (curIndex == total) {//预防索引越界
            curIndex--;
        } else if (curIndex < 0) {
            curIndex = 0;
        }
